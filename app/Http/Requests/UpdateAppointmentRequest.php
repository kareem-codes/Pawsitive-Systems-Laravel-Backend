...
class UpdateAppointmentRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        return [
            ...
            'type' => 'sometimes|in:checkup,surgery,vaccination,grooming,emergency,other',
    ...
    /**
     * Validate that the time slot is available (excluding current appointment).
     */
    protected function validateTimeSlot($appointmentDate, $fail): void
    {
        $veterinarianId = $this->veterinarian_id ?? $this->route('appointment')->veterinarian_id;
        ...
        $durationMinutes = $this->duration_minutes ?? $this->route('appointment')->duration_minutes ?? 30;
        $appointmentEnd = $appointmentStart->copy()->addMinutes($durationMinutes);

        // Check for conflicts (excluding current appointment)
        $conflicts = Appointment::where('veterinarian_id', $veterinarianId)
